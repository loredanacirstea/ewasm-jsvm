...
const initializeImports = (
    ...
    api.ethereum = {
        storeMemory: function (offset, bytes, {stack, position}) {
            ...
            logger.debug('MSTORE', [bytes, offset], [], getCache(), stack, changed, gasCost);
            return;
        },
        storeMemory8: function (offset, bytes, {stack, position}) {
            ...
            const changed = {memory: [offset.toNumber(), value, 1]}
            logger.debug('MSTORE8', [bytes, offset], [], getCache(), stack, changed, gasCost);
            return;
        },
        loadMemory: function (offset, {stack, position}){
            ...
            logger.debug('MLOAD', [offset], [result], getCache(), stack, changed, gasCost);
        ...
        getAddress: function ({stack, position}) {
            ...
            stack.push(address);
            logger.debug('ADDRESS', [], [address], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        // result is u128
        getExternalBalance: function (_address, {stack, position}){
            ...
            stack.push(balance);
            logger.debug('BALANCE', [_address], [balance], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        ...
        getBlockHash: function (number, {stack, position}) {
            ...
            stack.push(hash);
            logger.debug('BLOCKHASH', [number], [hash], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        ...
        call: function (
            ...
            logger.debug('CALL', [gas_limit,
                ...
                dataLength,
                outputOffset,
                outputLength,], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        callDataCopy: function (resultOffset, dataOffset, length, {stack, position}) {
            ...
            logger.debug('CALLDATACOPY', [resultOffset, dataOffset, length], [result], getCache(), stack, null, gasCost);
            return;
        },
        getCallDataSize: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('CALLDATASIZE', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        callDataLoad: function(dataOffset, {stack, position}) {
            ...
            stack.push(result);
            logger.debug('CALLDATALOAD', [dataOffset], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        // result i32 Returns 0 on success, 1 on failure and 2 on revert
        callCode: function (
            ...
            stack.push(result);
            logger.debug('CALLCODE', [gas_limit_i64, addressOffset_i32ptr_address, valueOffset_i32ptr_u128, dataOffset_i32ptr_bytes, dataLength_i32], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        // result i32 Returns 0 on success, 1 on failure and 2 on revert
        ...
        getCaller: function ({stack, position}) {
            ...
            stack.push(address);
            logger.debug('CALLER', [], [address], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        getCallValue: function ({stack, position}) {
            ...
            stack.push(value);
            logger.debug('CALLVALUE', [], [value], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        codeCopy: function (resultOffset, codeOffset, length, {stack, position}) {
            ...
            logger.debug('CODECOPY', [resultOffset, codeOffset, length], [], getCache(), stack, null, gasCost);
            return;
        },
        ...
        getCodeSize: function({stack, position}) {
            ...
            stack.push(result);
            logger.debug('CODESIZE', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        ...
        getBlockCoinbase: function({stack, position}) {
            ...
            stack.push(value);
            logger.debug('COINBASE', [], [value], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        create: function (
            value,
            ...
            logger.debug('CREATE', [value,
                dataOffset,
                dataLength,
            ], [address], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        getBlockDifficulty: function ({stack, position}) {
            ...
            stack.push(value);
            logger.debug('DIFFICULTY', [], [value], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        ...
        getExternalCodeSize: function (address, {stack, position}) {
            ...
            stack.push(result);
            logger.debug('EXTCODESIZE', [address], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        // result gasLeft i64
        getGasLeft: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('GAS', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        ...
        getBlockGasLimit: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('GASLIMIT', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        getTxGasPrice: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('GASPRICE', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        log: function (
            dataOffset,
        ...
        getBlockNumber: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('NUMBER', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        getTxOrigin: function ({stack, position}) {
            ...
            stack.push(address);
            logger.debug('ORIGIN', [], [address], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        finish: function (dataOffset, dataLength, {stack, position}) {
            ...
            logger.debug('FINISH', [dataOffset, dataLength], [result], getCache(), stack, null, gasCost);
        ...
        revert: function (dataOffset, dataLength, {stack, position}) {
            ...
            logger.debug('REVERT', [dataOffset, dataLength], [result], getCache(), stack, null, gasCost);
        ...
        getReturnDataSize: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('RETURNDATASIZE', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        returnDataCopy: function (resultOffset, dataOffset, length, {stack, position}) {
            ...
            logger.debug('RETURNDATACOPY', [resultOffset, dataOffset, length], [], getCache(), stack, null, gasCost);
            return;
        },
        selfDestruct: function (address, {stack, position}) {
            ...
            logger.debug('SELFDESTRUCT', [address], [], getCache(), stack, null, gasCost);
        ...
        getBlockTimestamp: function ({stack, position}) {
            ...
            stack.push(result);
            logger.debug('TIMESTAMP', [], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        return: (offset, length, {stack, position}) => {
            ...
            logger.debug('RETURN', [offset, length], [result], getCache(), stack, null, gasCost);
            finishAction({result, gas: jsvm_env.getGas()});
            return {stack, position: 0};
        },
        revert: (offset, length, {stack, position}) => {
            ...
            logger.debug('REVERT', [offset, length], [result], getCache(), stack, null, gasCost);
            revertAction({result, gas: jsvm_env.getGas()});
            return {stack, position: 0};
        },
        stop: ({stack, position}) => {
            ...
            jsvm_env.useGas(gasCost);
            return api.ethereum.return(toBN(0), toBN(0), stack, null, gasCost);
        },
        keccak256: (offset, length, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('keccak256', [offset, length], [result], getCache(), stack, null, gasCost);
            return {stack, position: 0};
        },
        ...
        add: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('ADD', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        mul: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('MUL', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        ...
        sub: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('SUB', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        div: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('DIV', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        sdiv: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('SDIV', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        mod: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('MOD', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        smod: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('SMOD', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        addmod: (a, b, c, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('ADDMOD', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        mulmod: (a, b, c, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('MULMOD', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        exp: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('EXP', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        signextend: (size, value, {stack, position}) => {
            ...
            const result = value;
            stack.push(result);
            logger.debug('SIGNEXTEND', [size, value], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        lt: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('LT', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        gt: (a, b, {stack, position}) => {
            ...
            result = toBN(result ? 1 : 0);
            stack.push(result);
            logger.debug('GT', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        slt: (a, b, {stack, position}) => {
            ...
            result = toBN(result ? 1 : 0);
            stack.push(result);
            logger.debug('SLT', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        sgt: (a, b, {stack, position}) => {
            ...
            result = toBN(result ? 1 : 0);
            stack.push(result);
            logger.debug('SGT', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        eq: (a, b, {stack, position}) => {
            ...
            result = toBN(result ? 1 : 0);
            stack.push(result);
            logger.debug('EQ', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        iszero: (a, {stack, position}) => {
            ...
            result = toBN(result ? 1 : 0);
            stack.push(result);
            logger.debug('ISZERO', [a], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        and: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('AND', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        or: (a, b, {stack, position}) => {
            ...
            const result = a.or(b);
            stack.push(result);
            logger.debug('OR', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        xor: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('XOR', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        not: (a, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('NOT', [a], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        byte: (nth, bb, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('BYTE', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        shl: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('SHL', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        shr: (a, b, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('SHR', [a, b], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        sar: (nobits, value, {stack, position}) => {
            ...
            stack.push(result);
            logger.debug('SAR', [nobits, value], [result], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        handlePush: (code, bytecode, {stack, position}) => {
            ...
            logger.debug('PUSH' + no + ' 0x' + value.toString(16).padStart(no*2, '0'), [value], [], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        handleSwap: (code, {stack, position}) => {
            ...
            logger.debug('SWAP' + no, [], [], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        handleDup: (code, {stack, position}) => {
            ...
            logger.debug('DUP' + no, [], [], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        jump: (newpos, {stack}) => {
            ...
            logger.debug('JUMP', [newpos], [], getCache(), stack, null, gasCost);
        ...
        jumpi: (newpos, condition, {stack, position}) => {
            ...
            logger.debug('JUMPI', [condition, newpos], [], getCache(), stack, null, gasCost);
            return {stack, position};
        },
        jumpdest:({stack}) => {
            const gasCost = getPrice('jumpdest');
            jsvm_env.useGas(gasCost);
            logger.debug('JUMPDEST', [], [], getCache(), stack, null, gasCost);
        },
        pop: ({stack, position}) => {
            const gasCost = getPrice('pop');
            jsvm_env.useGas(gasCost);
            stack.pop();
            logger.debug('POP', [], [], getCache(), stack, null, gasCost);
            return {stack, position};
        },
    }
